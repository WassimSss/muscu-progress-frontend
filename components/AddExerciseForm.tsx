...
export default function AddExerciseForm({ className, handleRefreshWorkouts }: AddExerciseFormProps): JSX.Element {
  ...
  useEffect(() => {
    fetch("https://vercel.com/wassimsss-projects/muscu-progress-backendusers/muscle-groups/get", {
      headers: {
  ...
  const handleMuscleGroupChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    fetch(`https://vercel.com/wassimsss-projects/muscu-progress-backendusers/exercises/get/${value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const handleSubmit = async () => {
    ...
    const addWokout = await fetch("https://vercel.com/wassimsss-projects/muscu-progress-backendusers/workouts/add", {
      method: "POST",
      headers: {
