...
export default function AddExerciseForm({ className, handleRefreshWorkouts }: AddExerciseFormProps): JSX.Element {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/users/muscle-groups/get", {
      headers: {
  ...
  const handleMuscleGroupChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    fetch(`http://localhost:8080/users/exercises/get/${value}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
  ...
  const handleSubmit = async () => {
    ...
    const addWokout = await fetch("http://localhost:8080/users/workouts/add", {
      method: "POST",
      headers: {
