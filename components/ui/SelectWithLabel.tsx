...
export default function SelectWithLabel({ label, name, required, className, option, onChange, active = true, text = null }: SelectWithLabelProps) {
  ...
  const optionJSX = [active ? {
    ...
    if (option) {
      return <option value={option.value} data-text={option.label} key={i}>{option.label}</option>
    }
    return null;
  });
  ...
  return (<div>
    ...
    <select id={name} required={required} disabled={!active} onChange={onChange}
      ...
      {active ? optionJSX : <option value="" disabled defaultValue={'null'}>Non disponible</option>}
    </select>
  </div >);
}